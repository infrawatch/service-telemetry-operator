...
curl -g "${PROMETHEUS}/api/v1/query?" --data-urlencode 'query=collectd_cpu_total{cpu="0",type="user",service="stf-default-collectd-telemetry-smartgateway",host="'"${POD}"'"}[1m]' 2>&2 | tee /tmp/query_output
echo; echo

# The egrep exit code is the result of the test and becomes the container/pod/job exit code
grep -E '"result":\[{"metric":{"__name__":"collectd_cpu_total","cpu":"0","endpoint":"prom-http","host":"'"${POD}"'","service":"stf-default-collectd-telemetry-smartgateway","type":"user"},"values":\[\[.+,".+"\]' /tmp/query_output
metrics_result=$?
